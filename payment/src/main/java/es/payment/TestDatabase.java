...
class TestDatabase{
  ...
  CommandLineRunner initDatabase(AccountRepository account_repository, TransactionRepository transaction_repository) {
    return args -> {
      ...
      log.info("Preloading " + transaction_repository.save(new Transaction(1234L,1, 2, "Ovos", 2310.50, "Completed", LocalDate.now())));
      log.info("Preloading " + transaction_repository.save(new Transaction(4321L,2, 1, "Bananas", 4201.20, "Pending", LocalDate.now())));
    };
  }
}
